...
1. **Cluster-wide HTTP-01 cert renewal is broken** (surfaced via the already-expired `static/ingress-letsencrypt`). Root cause: **ingress-nginx v1.12 defaults `strict-validate-path-type: true`, which rejects cert-manager's HTTP-01 solver Ingress** for the `/.well-known/acme-challenge/...` path (the challenge Ingress is never admitted). This is **pre-existing and unrelated to the K3s upgrade** — ingress-nginx v1.12.1 predates it, which is why `static` (first to hit renewal, ~April 2026) failed while the rest stayed `Ready`. **All 30 certs use the same issuer and will fail as they each reach renewal** — a slow cluster-wide outage. **Fix:** set `strict-validate-path-type: "false"` in the `ingress-nginx-controller` ConfigMap, then re-trigger the stuck Order. (Earlier hunch of a SAN/DNS/rate-limit cause was wrong — the challenges never reach the network stage.)
